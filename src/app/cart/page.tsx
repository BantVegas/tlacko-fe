...
export default function CartPage() {
  ...
  async function handleCheckout() {
    if (!items.length) {
      alert("Košík je prázdny. Najprv pridaj položku.");
      return;
    }

    const totalEur = Number(totalPrice.toFixed(2));
    // bezpečnostná brzda – niektoré metódy na Comgate nepovoľujú < 1.00 €
    if (totalEur < 1) {
      alert("Minimálna suma pre platbu je 1.00 € (zvýš počet alebo cenu).");
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data?.ok || !data?.redirect) {
        console.error("API /api/comgate-create", res.status, data);
        alert(data?.error || `Vytvorenie platby zlyhalo (HTTP ${res.status}).`);
        return;
      }
      ...
      alert("Chyba pri vytváraní platby. Skontroluj, či /api endpointy bežia na Verceli.");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {empty ? (
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <Link
              to="/"
              className="inline-block mt-4 px-5 py-3 rounded-xl bg-blue-600 text-white hover:bg-blue-700"
            >
              Pokračovať v nákupe
            </Link>
          </div>
